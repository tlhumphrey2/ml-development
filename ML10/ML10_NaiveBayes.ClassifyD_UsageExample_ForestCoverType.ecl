//ML10_NaiveBayes.ClassifyD_UsageExample_ForestCoverType.ecl
IMPORT * FROM $;
IMPORT ML;
IMPORT ML10;
...
dnf_train := nf_train0(value<>0);
dnf_test := nf_test0(value<>0);
...
Model3 := MyNaiveBayesRegression.LearnC(indep_dnf_train,dep_dnf_train);
OUTPUT(COUNT(Model3),NAMED('c_Model3'));
OUTPUT(SORT(Model3,id,number),NAMED('Model3'));

predict:=MyNaiveBayesRegression.ClassifyC(indep_dnf_test,Model3);
OUTPUT(predict,NAMED('predict'));

TestModule := MyNaiveBayesRegression.TestC(indep_dnf_test,dep_dnf_test);
TestModule.Raw;
...
/*
return_rec := RECORD
  REAL proportion_correct;
	DATASET(RECORDOF(Model3)) Model3;
END;

x:=DATASET([{proportion_correct,Model3}]
            ,return_rec
   );
OUTPUT(x,NAMED('x'));
*/