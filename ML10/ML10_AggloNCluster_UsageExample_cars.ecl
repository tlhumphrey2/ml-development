//ML10_AggloNCluster_UsageExample_cars.ecl
IMPORT * FROM ML;
car_rec := RECORD
    INTEGER	car_id;
    REAL	mpg;
...
OUTPUT((ML.Cluster.AggloN(dEntities,12,,ML.Cluster.c_Method.max_dist).Dendrogram)[1].members,NAMED('max_distDendrogram'));
OUTPUT((ML.Cluster.AggloN(dEntities,12,,ML.Cluster.c_Method.min_dist).Dendrogram)[1].members,NAMED('min_distDendrogram'));
OUTPUT((ML.Cluster.AggloN(dEntities,12,,ML.Cluster.c_Method.ave_dist).Dendrogram)[1].members,NAMED('ave_distDendrogram'));
