//ML10_NaiveBayes.ClassifyD_UsageExample_qualForestCoverType.ecl
IMPORT * FROM $;
IMPORT ML;
IMPORT ML10;
...
d := PROJECT(ML10.randomize(DATASET(filename,covtype_rec,THOR)),TRANSFORM(qual_covtype_rec,SELF.rid:=COUNTER,SELF:=LEFT));
...
predict:=MyNaiveBayesRegression.ClassifyD(indep_dnf_test,Model3) : PERSIST('thumphrey::qual_indep_dnf_test::predictions2');
