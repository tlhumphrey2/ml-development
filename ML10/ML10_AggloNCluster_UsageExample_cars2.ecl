//ML10_AggloNCluster_UsageExample_cars2.ecl
IMPORT * FROM ML;
IMPORT ML10;
car_rec := RECORD
    INTEGER	car_id;
...
// mtcars dataset (found in R as a default dataset)
d0:=DATASET([
...
// Natural fields of the above dataset (by natural, I mean values are continue, i.e. not categorical)
car_rec2 := RECORD
    INTEGER	car_id;
    REAL	mpg;
    REAL	disp;
...
// d is a standardized version of d1.
d:=PROJECT(d1
...
OUTPUT((ML10.Cluster.AggloN(dEntities,12,,ML.Cluster.c_Method.max_dist).Dendrogram)[1].members,NAMED('max_distDendrogram'));
OUTPUT((ML10.Cluster.AggloN(dEntities,12,,ML.Cluster.c_Method.min_dist).Dendrogram)[1].members,NAMED('min_distDendrogram'));
OUTPUT((ML10.Cluster.AggloN(dEntities,12,,ML.Cluster.c_Method.ave_dist).Dendrogram)[1].members,NAMED('ave_distDendrogram'));
