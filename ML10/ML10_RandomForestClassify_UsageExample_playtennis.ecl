//ML10_RandomForestClassify_UsageExample_playtennis.ecl
IMPORT * FROM ML;
...
ToField(weather_Data, pr_weather_Data);
indep_Data:= TABLE(weather_Data,{id, outlook, temperature, humidity, windy});
OUTPUT(indep_Data, NAMED('indep_TABLE_Data'));
dep_Data:= TABLE(weather_Data,{id, play});
OUTPUT(dep_Data, NAMED('dep_TABLE_Data'));
