...
// ave-dist - average of the components
  ...
  EXPORT AggloN(DATASET(ML.Types.NumericField) d,UNSIGNED4 N,DF.Default Dist=DF.QEuclidean, c_Method cm=c_Method.min_dist):= MODULE
    Distance:=Distances(d,d,Dist)(x<>y);
        ...
        Step(DATASET(ClusterRec) cd00, unsigned iter) := FUNCTION
            cd := cd00(Members='');
            cl := cd00(Members<>'');
            ...
            J1 := 
               JOIN(cl,tojoin
                    ...
                    ,TRANSFORM(ClusterRec
                               ,SELF.id := IF ( RIGHT.ClusterID<>0, 1, 0)
                               ,SELF := LEFT
                     )
                    ,LEFT OUTER,KEEP(1));
            ...
            xx:=IF(~EXISTS(cd),cl,J3+J2(id=0)+cd3);
            o_rec:= RECORD
               unsigned iter;
               string dsid;
               RECORDOF(cd00)
            END;
            y_cd00:=PROJECT(CD00,TRANSFORM(o_rec,SELF.iter:=iter,SELF.dsid:='00',SELF:=LEFT));
            y_tojoin:=PROJECT(tojoin,TRANSFORM(o_rec,SELF.iter:=iter,SELF.dsid:='2j',SELF:=LEFT));
            y_J1:=PROJECT(J1,TRANSFORM(o_rec,SELF.iter:=iter,SELF.dsid:='J1',SELF:=LEFT));
            y_J2:=PROJECT(J2,TRANSFORM(o_rec,SELF.iter:=iter,SELF.dsid:='J2',SELF:=LEFT));
            y_J3:=PROJECT(J3,TRANSFORM(o_rec,SELF.iter:=iter,SELF.dsid:='J3',SELF:=LEFT));
            y_cd3:=PROJECT(cd3,TRANSFORM(o_rec,SELF.iter:=iter,SELF.dsid:='c3',SELF:=LEFT));
            y_rslt:=PROJECT(xx,TRANSFORM(o_rec,SELF.iter:=iter,SELF.dsid:='99',SELF:=LEFT));
            yy := y_cd00+y_tojoin+y_J1+y_J2+y_J3+y_cd3+y_rslt;
            O:=OUTPUT(SORT(yy,dsid,clusterid,id),ALL);
            RETURN WHEN(xx,O);
        END;
    SHARED res := LOOP(dinit,N,Step(ROWS(LEFT),COUNTER));
